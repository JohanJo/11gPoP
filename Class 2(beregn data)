...
let z1 = Rad1 * sin(deg2rad Lat1)
                    //dens startaccerelation. her vil solen(GMsol) trække lidt i jorden
let ax0 = - GMsol / (Rad0 * Rad0 * Rad0) * x0
let ay0 = - GMsol / (Rad0 * Rad0 * Rad0) * y0
let az0 = - GMsol / (Rad0 * Rad0 * Rad0) * z0
                    //hastigheden fundet ved brug af de to positioner
...
let ry0 = y0
let rz0 = z0
                    //ny position fundet med vores hastighed * med vores tid (1dag)
...
printfn "rx1=%f" rx1
printfn "x1=%f" x1
//vi kan se at de to punkter er det samme dvs. at formlerne virker
//og nu hvor vi kender acceleration og hastigheden burde vi kunne finde den næste og den næste.
//alt det der kommer nedenunder er mine test
(*let mutable rx = rx1 + vx1 + deltaT
let mutable ry = ry1 + vy1 + deltaT
let mutable rz = rz1 + vz1 + deltaT
let r1sim = decimal (sqrt(float (rx1*rx1+ry1*ry1+rz1*rz1)))
...
//  vx <- vx + ax * deltaT
  ...
  //vz <- vz + az * deltaT
(*
  ax <- GMsol / (Rad1 * Rad1 * Rad1) * rx
  ay <- GMsol / (Rad1 * Rad1 * Rad1) * ry
  az <- GMsol / (Rad1 * Rad1 * Rad1) * rz
  rx <- rx + vx * deltaT
  ry <- ry + vy * deltaT
  rz <- rz + vz * deltaT
  printfn "%f" rx
