...
type Planet (Lon0, Lat0, Rad0, Lon1, Lat1, Rad1) = class
  ...
  member this.SimPlanet (dage:float) =
    while dag <= dage do
      ...
      this.Move()
      dag <- dag + deltaT
  member this.Color = rnd.Next(256), rnd.Next(256), rnd.Next(256)
end
...
let planets =
  [earth; sun; neptun; venus; jupiter; mars; mercury; pluto; saturn; uranus]
...
type Animation (planets : List<Planet>) = class
  ...
  let createForm backColor (width, height) title draw =
    ...
    win.Paint.Add draw
    List.map (fun p-> win.Paint.Add (fun e-> draw e.Graphics.DrawLines (new System.Drawing.Pen (planet.Color) , Planet.GetAU2P)) planets
    win
  let lone =
  let drawUniverse (e : PaintEventArgs) =
    let AU2Pixel (AU:float ) =
      (float (fst size) / (2.0*33.0)) * AU + (float(fst size) / 2.0)
    let drawPlanet (planet : Planet) =
      let drawPixel (pos : Position) =
        match pos with
        | {Position.x=x;Position.y=y;} ->
          let rect = new Rectangle (int(x) + 400, int(y) + 200, 10, 10)
          let aBrush : SolidBrush = new SolidBrush (Color.FromArgb(planet.Color,0))
          e.Graphics.FillEllipse (aBrush ,int(x), int(y), 2 ,2)
      let planetPos = planet.listpos |> List.map (fun {Position.x=x;Position.y=y;Position.z=z} -> {Position.x=AU2Pixel(x); Position.y=AU2Pixel(y);Position.z=AU2Pixel(z)})
      List.map drawPixel planetPos |> ignore
    List.map drawPlanet planets


  member this.create () =
    let win = createForm backColor size title drawUniverse
